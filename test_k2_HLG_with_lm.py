...
if __name__ == "__main__":
    ...
    if device == torch.device('cpu'):
        d = torch.load(lang_dir / 'G_4_gram.pt', map_location=torch.device('cpu'))
    else:
        print(str(lang_dir/'G_4_gram.pt'))
    ...
    for lm_scale in lm_scale_list:
        ...
        print('Using the lm_scale {} for decoding...'.format(lm_scale))
        ...
        for k in ['test-clean']:
            ...
            with torch.no_grad():
                for batch in tqdm(dataloader):
                    ...
                    if use_whole_lattice:
                        ...
                        # best_path_dict = rescore_with_n_best_list(lattices, G, args.num_paths, lm_scale_list)
                        # for lm_scale_str, best_paths in best_path_dict.items():
                        #     hyps = get_texts(best_paths, indices)
                        #     hyps = ' '.join([symbols.get(x) for x in hyps[0]])
            ...
            if k == 'test-clean':
                if device == torch.device('cuda'):
                    with open('different_lm_scale_results_num_paths/k2-encode_out_test_clean_bs_{}_HLG_{}_num_paths_lm_scale_{}_gpu_G_4-pt.txt'.format(bs, args.num_paths, lm_scale), 'w') as w:
                        wer_metric.write_stats(w)
                else:
                    with open('k2-encode_out_test_clean_bs_{}_HLG_use_whole_lattices_lm_scale_1.2_cpu.txt'.format(bs), 'w') as w:
                        wer_metric.write_stats(w)
            if k == 'test-other':
                if device == torch.device('cuda'):
                    with open('different_lm_scale_results/k2-encode_out_test_other_bs_{}_HLG_use_whole_lattices_lm_scale_{}_gpu_G_4-pt.txt'.format(bs, lm_scale), 'w') as w:
                        wer_metric.write_stats(w)
                else:
                    with open('k2-encode_out_test_other_bs_{}_HLG_use_whole_lattices_lm_scale_1.2_cpu.txt'.format(bs), 'w') as w:
                        wer_metric.write_stats(w)
