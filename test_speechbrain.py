...
def load_model():
    ...
    ###without LM###
    model.hparams.lm_weight = 0.0
    model.hparams.ctc_weight_decode = 0.0
    model.hparams.test_beam_size = 1

    ###with LM###
    #model.hparams.lm_weight = 0.6
    #model.hparams.ctc_weight_decode = 0.4
    #model.hparams.test_beam_size = 66
    ...
    for k in test_datasets.keys():
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                ...
                predicted_words = [model.tokenizer.decode_ids(token_seq) for token_seq in predicted_tokens]

                ref = batch['wrd']

                predicted_words = [predicted_words[0].split(' ')]
                target_words = [str(ref[0]).split(' ')]

                wer_metric.append(idx, predicted_words, target_words)

        if k == 'test-clean':
            with open('speechbrain-test-clean-lm_weight_0-ctc_weight_0-bs_1.txt', 'w') as w:
                wer_metric.write_stats(w)
        
        if k == 'test-other':
            with open('speechbrain-test-other-lm_weight_0-ctc_weight_0-bs_1.txt', 'w') as w:
                wer_metric.write_stats(w)
